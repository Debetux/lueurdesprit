...
def home(request):
    ...
    latest_staffreviews_theatre = Work.objects.filter(type_of_work__slug="piece-de-theatre").exclude(staffreview__pk__isnull=True).order_by('-pk')[:10]
    latest_staffreviews_book = Work.objects.filter(type_of_work__slug="litterature").exclude(staffreview__pk__isnull=True).order_by('-pk')[:10]
    latest_staffreviews_cinema = Work.objects.filter(type_of_work__slug="cinema").exclude(staffreview__pk__isnull=True).order_by('-pk')[:10]
    latest_staffreviews_music = Work.objects.filter(type_of_work__slug="musique").exclude(staffreview__pk__isnull=True).order_by('-pk')[:10]
